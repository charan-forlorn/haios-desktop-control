...
$EvidenceRoot = Join-Path $Root "evidence\m02\$RunId"
$RuntimeRoot = Join-Path $Root "runtime\m02-$RunId"
$Port = 8772
...
Write-Host "[3] Dependency and pin identity"
...
if (-not (Select-String -Quiet -Path "src\upstream.ts" -Pattern 'DESKTOP_COMMANDER_VERSION = "0.2.47"')) {
    throw "DESKTOP_COMMANDER_PIN_MISMATCH"
}

if (Test-Port 8771) { throw "Q2R1_PORT_8771_NOT_FREE" }
if (Test-Port $Port) { throw "M02_PORT_8772_NOT_FREE" }
...
const key = process.env.HAIOS_M02_API_KEY;
if (!key) throw new Error("HAIOS_M02_API_KEY missing");
...
console.log(`M02_RUNTIME_READY=${address.url}`);
...
@'
const client = new Client({ name: "haios-m02-qualifier", version: "1.0.0" });
...
const executeCalls = [];
const digestPairs = [];
const call = async (name, args = {}, kind = "read", label = name) => {
  const result = await client.callTool({ name, arguments: args });
  if (result.isError) throw new Error(`LIVE_CALL_FAILED:${label}`);
  const text = result.content?.filter?.(item => item.type === "text").map(item => item.text ?? "").join("\n") ?? "";
  const payload = text ? JSON.parse(text) : {};
  if (kind === "execute") {
    if (payload.decision !== "ALLOW" || payload.exitCode !== 0) throw new Error(`EXECUTE_NOT_ALLOWED:${label}`);
    if (!/^[a-f0-9]{64}$/.test(payload.preStateDigest ?? "")) throw new Error(`PRE_DIGEST_INVALID:${label}`);
    if (payload.preStateDigest !== payload.postStateDigest) throw new Error(`CURRENTNESS_DRIFT:${label}`);
    executeCalls.push(label);
    digestPairs.push({ name: label, pre: payload.preStateDigest, post: payload.postStateDigest });
  } else {
    readCalls.push(name);
  }
...
await call("filesystem_read_multiple", { paths: [
  "C:\\Workspace\\haios-desktop-control\\package.json",
  "C:\\Workspace\\haios-desktop-control\\tsconfig.json",
] });
await call("filesystem_stat", { path: "C:\\Workspace\\haios-desktop-control\\package.json" });
const started = await call("search_start", {
  path: "C:\\Workspace\\haios-desktop-control", pattern: "*.ts", searchType: "files", maxResults: 5, contextLines: 0,
});
const sessionMatch = JSON.stringify(started.result).match(/search_[A-Za-z0-9_-]+/);
...
'@ | Add-Content -Encoding utf8 $ClientScript
@'
await call("project_test", {}, "execute");
await call("project_typecheck", {}, "execute");
await call("project_build", {}, "execute");
await call("git_status", {}, "execute");
await call("git_diff", { mode: "working" }, "execute", "git_diff:working");
await call("git_diff", { mode: "staged" }, "execute", "git_diff:staged");
await call("git_log", { maxCount: 5 }, "execute");

const rawDenied = await client.callTool({ name: "start_process", arguments: {} });
if (!rawDenied.isError) throw new Error("RAW_START_PROCESS_CALL_NOT_DENIED");

const expected = [
  "desktop_status","gateway_status","filesystem_list","filesystem_read","filesystem_read_multiple","filesystem_stat",
  "search_start","search_results","search_stop","search_list","process_list","session_list",
  "project_test","project_typecheck","project_build","git_status","git_diff","git_log",
];
const names = tools.tools.map(t => t.name);
if (JSON.stringify(names) !== JSON.stringify(expected)) throw new Error("LIVE_TOOL_SURFACE_MISMATCH");
if (readCalls.length !== 12) throw new Error("LIVE_READ_COVERAGE_INCOMPLETE");
if (new Set(executeCalls.map(name => name.split(":")[0])).size !== 6) throw new Error("LIVE_EXECUTE_COVERAGE_INCOMPLETE");
console.log(JSON.stringify({ toolNames: names, readCalls, executeCalls, digestPairs, rawStartProcessDenied: true }));
await client.close();
'@ | Add-Content -Encoding utf8 $ClientScript
...
try {
    ...
    Remove-Item Env:HAIOS_M02_API_KEY -ErrorAction SilentlyContinue
    ...
    Write-Host "[6] Live MCP READ + CONTROLLED EXECUTE qualification"
    $env:HAIOS_M02_API_KEY = $Key
    ...
    if ($LiveResult.toolNames.Count -ne 18) { throw "LIVE_TOOL_COUNT_MISMATCH" }
    if ($LiveResult.readCalls.Count -ne 12) { throw "LIVE_READ_COVERAGE_INCOMPLETE" }
    if ($LiveResult.executeCalls.Count -ne 7) { throw "LIVE_EXECUTE_VARIANT_COVERAGE_INCOMPLETE" }
    if ($LiveResult.digestPairs.Count -ne 7) { throw "LIVE_DIGEST_COVERAGE_INCOMPLETE" }
    if (-not $LiveResult.rawStartProcessDenied) { throw "RAW_START_PROCESS_NEGATIVE_FAILED" }
    foreach ($Pair in $LiveResult.digestPairs) {
        if ($Pair.pre -ne $Pair.post) { throw "LIVE_CURRENTNESS_DRIFT:$($Pair.name)" }
    }
    Write-Host "LIVE_TOOLS_LIST=18/18"
    Write-Host "LIVE_READ_CALLS=12/12"
    Write-Host "LIVE_EXECUTE_CALLS=7/7_VARIANTS_6_UNIQUE"
    Write-Host "LIVE_CURRENTNESS_DIGESTS=PASS"
    Write-Host "LIVE_RAW_START_PROCESS_DENIAL=PASS"
}
finally {
    Remove-Item Env:HAIOS_M02_API_KEY -ErrorAction SilentlyContinue
    ...
    if (Test-Port $Port) { throw "M02_PORT_8772_CLEANUP_FAILED" }
...
$Result = [ordered]@{
    ...
    downstream_tool_count = 18
    downstream_tool_names = @($LiveResult.toolNames)
    live_read_calls = @($LiveResult.readCalls)
    live_execute_calls = @($LiveResult.executeCalls)
    currentness_digest_pairs = @($LiveResult.digestPairs)
    read_capability = "QUALIFIED"
    execute_capability = "QUALIFIED_CANDIDATE"
...
$ResultPath = Join-Path $EvidenceRoot "m02-qualification-result.json"
$Result | ConvertTo-Json -Depth 10 | Set-Content -Encoding utf8 $ResultPath
Write-Host "RESULT_PATH=$ResultPath"
Write-Host "HAIOS_DESKTOP_CONTROL_PLANE_R1_M02_READY_FOR_INDEPENDENT_VERIFICATION" -ForegroundColor Green
