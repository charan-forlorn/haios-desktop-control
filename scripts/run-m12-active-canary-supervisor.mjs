...
const STRICT_RUNTIME_LAUNCHER = resolve(HERE, "run-m12-active-canary-runtime.mjs");
...
export async function superviseM12Runtime({
  configPath,
  signal,
  ...
  if (!configPath) throw new Error("M12_SUPERVISOR_CONFIG_REQUIRED");
  if (!signal) throw new Error("M12_SUPERVISOR_SIGNAL_REQUIRED");
  if (!Number.isInteger(maxRestarts) || maxRestarts < 0 || maxRestarts > 10) {
    throw new Error("M12_SUPERVISOR_RESTART_BOUND_INVALID");
  }
  if (!Number.isInteger(backoffMs) || backoffMs < 0 || backoffMs > 30000) {
    throw new Error("M12_SUPERVISOR_BACKOFF_INVALID");
  }
...
function expectedM12ConfigPath() {
  const localAppData = process.env.LOCALAPPDATA;
  if (!localAppData) throw new Error("M12_SUPERVISOR_LOCALAPPDATA_REQUIRED");
  return resolve(localAppData, "HAIOS", "M12", "host-config.json");
}

async function main() {
  if (process.argv.length !== 3) throw new Error("M12_SUPERVISOR_ARGS_REQUIRED");
  const suppliedConfig = resolve(process.argv[2]);
  const expectedConfig = expectedM12ConfigPath();
  if (suppliedConfig.toLowerCase() !== expectedConfig.toLowerCase()) {
    throw new Error("M12_SUPERVISOR_CONFIG_REJECTED");
  }
  ...
  try {
    return await superviseM12Runtime({ configPath: suppliedConfig, signal: controller.signal });
  } finally {
