...
if (!process.argv[2] || !process.argv[3]) throw new Error("M11_HOST_PROBE_ARGS_REQUIRED");

const endpoint = new URL("http://127.0.0.1:8769/mcp");
const apiKey = await loadHostApiKey(apiKeyFile);
const client = new Client({ name: "m11-active-canary-host-probe", version: "1.0.0" });
...
try {
  ...
  const denied = await call("operator_begin_transaction", {
    projectId: "m11-denied",
    canonicalRoot: "C:\\m11-denied",
  });
  ...
  if (!pass) throw new Error("M11_HOST_PROBE_POLICY_FAILED");
