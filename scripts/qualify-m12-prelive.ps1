...
$full_test_sha256=Invoke-Logged "full-regression" {& npx.cmd vitest run --passWithNoTests --exclude 'dist/**' --maxWorkers=1 --minWorkers=1}
...
$activationMembers=[ordered]@{preflight_m12_sha256=Get-Sha256 (Join-Path $Root "scripts\preflight-m12-b5-activation.ps1");executor_sha256=Get-Sha256 (Join-Path $Root "scripts\execute-m12-b5-activation.ps1");rollback_sha256=Get-Sha256 (Join-Path $Root "scripts\rollback-m12-b5-to-m11.ps1");probe_sha256=Get-Sha256 (Join-Path $Root "scripts\probe-m12-b5-host.mjs")}
