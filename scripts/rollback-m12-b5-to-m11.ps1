...
$result=[ordered]@{terminal="HAIOS_DESKTOP_CONTROL_PLANE_R1_M12_ROLLED_BACK_TO_CERTIFIED_M11_ACTIVE_CANARY";m11_task=$M11Task;m11_mode="ACTIVE";m10_api_key_rotated=$false;canary_root=$CanaryRoot;canary_head=[string]$Envelope.canary_head;tunnel_route=$ExpectedTunnelRoute;listener_8768=@(Get-ListenerIdentity $SecurePort)};Write-JsonNoBom (Join-Path $EvidenceRoot "m12-rollback-result.json") $result
