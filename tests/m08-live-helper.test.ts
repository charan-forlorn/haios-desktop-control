...
describe("M08 disposable ACTIVE live helper contract", () => {
  it("builds the certified runtime stack and routes execution through MCP", async () => {
    const source = await readFile(helper, "utf8");
    for (const marker of [
      "createGatewayServer", "OperatorTransactionService", "LocalOperatorGit",
      "OperatorTaskRunner", "SandboxExecutor", "loadTaskRegistryV2", "loadTaskEffectPolicy",
  ...
  it("contains separate promotion, rollback, and stale-CAS scenarios with no forbidden remote authority", async () => {
    const source = await readFile(helper, "utf8");
    for (const marker of ["promotionPassed", "rollbackPassed", "staleCasDenied", "canonicalUnchangedBeforePromotion", "worktreeResidueZero"]) {
      expect(source).toContain(marker);
    }
    for (const forbidden of ["git push", "git fetch", "git pull", "docker pull", "OPENAI_API_KEY", "GITHUB_TOKEN", "8768/mcp", "8769/mcp"]) {
