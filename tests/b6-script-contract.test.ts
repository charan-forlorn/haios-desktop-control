...
describe("B6 staged activation helper boundaries", () => {
  ...
  it("moves verified runtime bytes into an ACL-locked private execution root before import", async () => {
    ...
    expect(launcher).toContain("copyLockedProductionDependencies(executionRoot)");
    ...
    expect(attestation).toContain("lockedProductionDependencyFacts()");
    ...
    expect(launcher).toContain("toolchainSha256");
    expect(attestation).toContain("toolchainSha256");
    const lockedDeps = await readFile(join(process.cwd(), "scripts", "b6-lockfile-dependencies.mjs"), "utf8");
    const prepare = await readFile(join(process.cwd(), "scripts", "prepare-b6-runtime-build.mjs"), "utf8");
    for (const marker of ["npm_config_offline", "npm_config_ignore_scripts", "--offline", "--ignore-scripts", "--omit=dev", "metadata?.integrity", "copyLockedProductionDependencies", "lockedProductionDependencyFacts"]) expect(lockedDeps).toContain(marker);
    expect(lockedDeps).not.toContain("copyCurrentProductionDependencies");
    expect(prepare).toContain("withLockedBuildToolchain");
    expect(prepare).toContain("toolchainSha256");
    expect(prepare).not.toContain('join(repoRoot, "node_modules", "typescript"');
