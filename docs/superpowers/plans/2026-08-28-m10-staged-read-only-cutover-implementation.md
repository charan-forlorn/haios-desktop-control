...
- [ ] **Step 5: Require automatic rollback on any forward failure** after mutation begins; rollback stops/removes only the M10 task, proves host `:8769` is free, restores the old Operator host publish and verifies `READ_ONLY_EMERGENCY`, restores/recreates the dedicated tunnel from sealed preimage bindings and verifies its old route/mount/health, proves shared tunnel and `:8768` preservation, removes only M10-owned deployment/state residue, then re-verifies preservation before emitting the rollback terminal.
